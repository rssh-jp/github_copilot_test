...
void TouchInputHandler::handleMotionEvent(const GameActivityMotionEvent& motionEvent) {
    ...
    aout << "TouchInputHandler received event - action: " << (action & AMOTION_EVENT_ACTION_MASK) 
         << ", pointerCount: " << motionEvent.pointerCount << ", activeCount: " << activeTouches_.size() << std::endl;
    ...
    switch (action & AMOTION_EVENT_ACTION_MASK) {
        ...
        case AMOTION_EVENT_ACTION_POINTER_DOWN: {
            ...
            if (activeTouches_.size() == 1) {
                ...
                // 2本目の指 - ピンチ開始の可能性
                aout << "DEBUG: Pinch mode detected! Starting pinch processing..." << std::endl;
                ...
                // 初期ピンチ距離を計算
                initialPinchDistance_ = calculateDistance(activeTouches_[0], activeTouches_[1]);
                lastPinchDistance_ = initialPinchDistance_;
                
                // ピンチ開始イベントを即座に送信（ユニット移動を無効化するため）
                if (touchEventCallback_ && !pinchStartTriggered_) {
                    float centerX, centerY;
                    calculatePinchCenter(activeTouches_[0], activeTouches_[1], centerX, centerY);
                    aout << "DEBUG: Pinch start event - center: (" << centerX << ", " << centerY << "), scale: 1.0" << std::endl;
                    TouchEvent event(TouchInputType::PINCH_ZOOM, centerX, centerY, 1.0f); // スケール1.0で開始
                    touchEventCallback_(event);
                    pinchStartTriggered_ = true;
                }
                
                aout << "Pinch started, initial distance: " << initialPinchDistance_ << std::endl;
            } else {
                // 3本目以降は無視（現在の実装では）
                isLongTapCandidate_ = false;
                longTapTriggered_ = false;
        ...
        case AMOTION_EVENT_ACTION_MOVE: {
            if (isPinching_ && activeTouches_.size() >= 2) {
                // ピンチ中の処理
                // 最新の2つのタッチポイントを更新
                for (auto index = 0; index < motionEvent.pointerCount && index < 2; index++) {
                ...
                // 現在の距離を計算
                float currentDistance = calculateDistance(activeTouches_[0], activeTouches_[1]);
                aout << "DEBUG: Pinch move - currentDist: " << currentDistance << ", lastDist: " << lastPinchDistance_ 
                ...
                if (std::abs(currentDistance - lastPinchDistance_) > PINCH_THRESHOLD) {
                    ...
                    aout << "DEBUG: Sending PINCH_ZOOM event - scale: " << scale << " (currentDist: " << currentDistance << ", lastDist: " << lastPinchDistance_ << ")" << std::endl;
                    float centerX, centerY;
                    calculatePinchCenter(activeTouches_[0], activeTouches_[1], centerX, centerY);
                    
                    if (touchEventCallback_) {
                        TouchEvent event(TouchInputType::PINCH_ZOOM, centerX, centerY, scale);
                        aout << "CALLBACK_DEBUG: Calling touchEventCallback_ for PINCH_ZOOM" << std::endl;
                        touchEventCallback_(event);
                        aout << "CALLBACK_DEBUG: touchEventCallback_ call completed" << std::endl;
                    } else {
                        aout << "CALLBACK_DEBUG: touchEventCallback_ is null!" << std::endl;
                    }
                    
                    lastPinchDistance_ = currentDistance;
                    aout << "Pinch zoom, scale: " << scale << " center: (" << centerX << ", " << centerY << ")" << std::endl;
                }
            ...
            if (activeTouches_.size() == 1 && !isPinching_) {
                ...
                if (touchEventCallback_) {
                    if (longTapTriggered_) {
                        ...
                        TouchEvent event(TouchInputType::LONG_TAP_END, initialTouchX_, initialTouchY_);
                        touchEventCallback_(event);
                        aout << "Long tap ended, finger released" << std::endl;
                    } else if (isLongTapCandidate_) {
                        // ロングタップ時間に達する前に離された - ショートタップ
                        aout << "DEBUG: Generating SHORT_TAP event at (" << initialTouchX_ << ", " << initialTouchY_ << ")" << std::endl;
                        if (touchEventCallback_) {
                            TouchEvent event(TouchInputType::SHORT_TAP, initialTouchX_, initialTouchY_);
                            aout << "DEBUG: Calling touchEventCallback_ for SHORT_TAP" << std::endl;
                            touchEventCallback_(event);
                            aout << "DEBUG: touchEventCallback_ call completed for SHORT_TAP" << std::endl;
                        } else {
                            aout << "ERROR: touchEventCallback_ is null for SHORT_TAP!" << std::endl;
                        }
                        aout << "Short tap detected at (" << initialTouchX_ << ", " << initialTouchY_ << ")" << std::endl;
                    }
                }
...
float TouchInputHandler::calculateDistance(const TouchPoint& p1, const TouchPoint& p2) const {
    float dx = p1.x - p2.x;
    float dy = p1.y - p2.y;
    return std::sqrt(dx * dx + dy * dy);
}

void TouchInputHandler::calculatePinchCenter(const TouchPoint& p1, const TouchPoint& p2, float& centerX, float& centerY) const {
...
void TouchInputHandler::checkLongTap() {
    ...
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastTouchStart_).count();
    ...
    if (duration >= LONG_TAP_THRESHOLD_MS && touchEventCallback_) {
        ...
        aout << "Long tap triggered while holding at (" << initialTouchX_ << ", " << initialTouchY_ << ")" << std::endl;
    }
}
...
void TouchInputHandler::removeTouchPoint(int pointerId) {
    activeTouches_.erase(
        std::remove_if(activeTouches_.begin(), activeTouches_.end(),
                      [pointerId](const TouchPoint& touch) { return touch.id == pointerId; }),
        activeTouches_.end()
    );
}