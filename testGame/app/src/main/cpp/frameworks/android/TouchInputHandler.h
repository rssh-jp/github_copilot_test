...
struct TouchPoint {
    ...
    TouchPoint(int pointerId, float posX, float posY) 
        : id(pointerId), x(posX), y(posY), timestamp(std::chrono::steady_clock::now()) {}
};
...
private:
    ...
    // 設定可能な閾値
    static constexpr float LONG_TAP_THRESHOLD_MS = 500.0f;  // ロングタップ判定時間（ミリ秒）
    static constexpr float TOUCH_MOVE_THRESHOLD = 20.0f;    // タッチ移動判定の閾値（ピクセル）
    static constexpr float PINCH_THRESHOLD = 2.0f;         // ピンチ判定の閾値（ピクセル）- 敏感に調整
     ...
     * @brief ピンチの中心点を計算
     */
    void calculatePinchCenter(const TouchPoint& p1, const TouchPoint& p2, float& centerX, float& centerY) const;
