...
public:
   ...
   * @param b 青成分 (0.0～1.0)
   */
  void registerUnitWithColor(const std::shared_ptr<UnitEntity> &unit, float r,
                             float g, float b);
   ...
   */
  void moveAllUnitsToRandomInView(float minx, float miny, float maxx,
                                  float maxy);

  /**
   * @brief Reset all registered units to their recorded initial positions
   * (teleport)
   */
   ...
   */
  void renderAttackRanges(const Shader *shader);
...
private:
  ...
  std::unordered_map<std::string, std::shared_ptr<TextureAsset>>
      colorTextureCache_;
