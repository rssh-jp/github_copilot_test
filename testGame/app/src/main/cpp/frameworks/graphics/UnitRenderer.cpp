 ...
 *
 * Responsibilities:
 * - Visualize UnitEntity instances: model, HP bars, debug wireframes (collision
 * / attack ranges).
 * - Provide a registration API for units and lightweight per-frame
 * update/render calls.
 *
 * Notes:
 * - Keep rendering logic here; do not move game rules into rendering code. The
 * renderer may read domain state but must not mutate business logic.
 */
#include "../domain/services/CollisionDomainService.h"
#include "android/AndroidOut.h"
#include <cmath>
...
UnitRenderer::~UnitRenderer() { aout << "UnitRenderer destroyed" << std::endl; }
 ...
 */
void UnitRenderer::setShowAttackRanges(bool show) { showAttackRanges_ = show; }

/**
 * @brief ユニットをレンダラーに登録します。
 *
 * 登録後は render()/updateUnits()
 * の対象となり、内部でユニットに対応するテクスチャが 設定されます。
 */
...
void UnitRenderer::registerUnitWithColor(
    const std::shared_ptr<UnitEntity> &unit, float r, float g, float b) {
  if (unit) {
    units_[unit->getId()] = unit;
...
void UnitRenderer::unregisterUnit(int unitId) {
  ...
  if (it != units_.end()) {
    aout << "Unregistered unit: " << it->second->getName() << " (ID: " << unitId
         << ")" << std::endl;
    units_.erase(it);
  }
}
...
std::shared_ptr<TextureAsset> UnitRenderer::getColorTexture(float r, float g,
                                                            float b) {
  // RGB値をキーにして、すでに同じ色のテクスチャがキャッシュにあるか確認
  std::string colorKey =
      std::to_string(r) + "_" + std::to_string(g) + "_" + std::to_string(b);
...
void UnitRenderer::render(const Shader *shader, float cameraZoom) {
  for (const auto &pair : units_) {
    ...
    if (!unit->isAlive()) {
      ...
      aout << unit->getName() << " is attacking - showing orange highlight"
           << std::endl;
    ...
    // ユニットのワールド座標位置を設定（ビュー行列でカメラ変換が適用される）
 ...
 */
void UnitRenderer::renderAttackRanges(const Shader *shader) {
  ...
  for (const auto &pair : units_) {
    ...
    auto rangeTexture = getColorTexture(
        lr * alphaMultiplier, lg * alphaMultiplier, lb * alphaMultiplier);
...
/**
 * @brief 各ユニットの当たり判定（collision
 * radius）をワイヤーフレームで描画します。
 *
 * デバッグ用途。描画は GL_LINE_LOOP
 * で行われ、深度書き込みをオフにして最前面に表示します。
 */
...
void UnitRenderer::updateUnits(float deltaTime) {
  ...
  for (auto &pair : units_) {
    ...
    // TODO: UnitEntityには avoidCollisions
  ...
  // 衝突予測機能付きでユニットを更新
  // 注意: 移動更新は MovementUseCase で地形倍率を考慮して行われるため、
  // ここでは unit->updateMovement(deltaTime) を呼ばない
  // （呼ぶと地形無視で上書きされてしまう）
  
  // TODO: 将来的に攻撃などの更新処理をここで実装する場合は
  // 移動処理を含まない update メソッドを呼ぶこと
}
...
void UnitRenderer::renderHPBar(const Shader *shader,
  ...
  // HPバーの背景（灰色）
  {
    // バーの背景用のモデル
    std::vector<Vertex> vertices = {
        Vertex(Vector3{barWidth / 2, barY + barHeight, 0.1f},
               Vector2{1, 0}), // 右上
        Vertex(Vector3{-barWidth / 2, barY + barHeight, 0.1f},
               Vector2{0, 0}),                                     // 左上
  ...
  if (hpRatio > 0) {
    ...
    // バーのモデル
    std::vector<Vertex> vertices = {
        Vertex(Vector3{leftX + currentWidth, barY + barHeight, 0.2f},
               Vector2{1, 0}),                                         // 右上
...
Model UnitRenderer::createUnitModel() {
  ...
  aout << "Created red texture with ID: " << redTexture->getTextureID()
       << std::endl;
 ...
 */
const std::unordered_map<int, std::shared_ptr<UnitEntity>> &
UnitRenderer::getAllUnits() const {
  return units_;
}

// Helper to move units to random positions within provided world bounds
static void moveUnitsRandomHelper(
    const std::unordered_map<int, std::shared_ptr<UnitEntity>> &units,
...
void UnitRenderer::moveAllUnitsToRandomInView(float minx, float miny,
                                              float maxx, float maxy) {
  moveUnitsRandomHelper(units_, minx, miny, maxx, maxy);
}

// Reset all units that have an initial position recorded to that position
// (instant teleport)
