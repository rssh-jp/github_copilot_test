...
constexpr float kStopBuffer =
    0.02f; // distance to stay away from blocking tiles
} // namespace
...
Position GameMap::resolveMovementTarget(const Position &start,
  ...
  const int sampleCount =
      std::max(1, static_cast<int>(std::ceil(distance / sampleLength)));
  ...
  for (int i = 1; i <= sampleCount; ++i) {
    ...
    if (!isWalkable(sample, radius)) {
      ...
      Position adjusted(lastWalkable.getX() - offsetX,
                        lastWalkable.getY() - offsetY);
