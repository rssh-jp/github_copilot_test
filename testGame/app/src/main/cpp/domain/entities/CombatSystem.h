 ...
 *
 * Safety notes:
 * - This class mutates UnitEntity state (enterCombat/exitCombat/takeDamage).
 * Callers should perform validation (alive checks, faction checks) before
 * invoking executeCombat.
 */
...
struct CombatResult {
  ...
  CombatResult()
      : attackerWon(false), damageDealt(0), damageReceived(0),
...
public:
  ...
  CombatResult executeCombat(UnitEntity &attacker, UnitEntity &defender) {
    ...
    // ダメージ計算
    int attackerDamage =
        calculateDamage(attacker.getStats(), defender.getStats());
    int defenderDamage =
        calculateCounterDamage(defender.getStats(), attacker.getStats());
  ...
  std::vector<std::shared_ptr<UnitEntity>> findTargetsInRange(
    ...
    for (const auto &target : potentialTargets) {
      if (target && target->isAlive() && target->getId() != attacker.getId() &&
  ...
  std::shared_ptr<UnitEntity> selectNearestTarget(
    ...
    float nearestDistance =
        attacker.getPosition().distanceTo(nearest->getPosition());

    for (size_t i = 1; i < targets.size(); ++i) {
      float distance =
          attacker.getPosition().distanceTo(targets[i]->getPosition());
   ...
   * @return 計算されたダメージ
   */
  int calculateDamage(const UnitStats &attackerStats,
                      const UnitStats &defenderStats) const {
    // 基本ダメージ
   ...
   * @return 反撃ダメージ
   */
  int calculateCounterDamage(const UnitStats &defenderStats,
                             const UnitStats &attackerStats) const {
    // 反撃は基本攻撃力の50%
