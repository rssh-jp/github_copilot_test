...
public:
   ...
   */
  static bool
  canMoveTo(const UnitEntity &unit, const Position &targetPosition,
   ...
   */
  static bool
  hasCollisionAt(const Position &position,
                 const std::vector<std::shared_ptr<UnitEntity>> &allUnits,
   ...
   */
  static bool
  hasCollisionOnPath(const Position &start, const Position &end,
                     const std::vector<std::shared_ptr<UnitEntity>> &allUnits,
                     const UnitEntity *excludeUnit = nullptr,
                     float movingRadius = 0.0f);
   ...
   * @param outContactPosition
   * 最初に接触する位置を返す（見つからない場合は不変）
   * @param outContactUnit 接触したユニットへのポインタ（見つからない場合は
   * nullptr）
  ...
  static bool findFirstContactOnPath(
      ...
      const UnitEntity *excludeUnit = nullptr, float movingRadius = 0.0f);
...
private:
   ...
   * @brief 点と円の衝突判定
   */
  static bool isPointInCircle(const Position &point,
                              const Position &circleCenter, float radius);

  /**
   * @brief 線分と円の衝突判定
   */
  static bool isLineSegmentIntersectingCircle(const Position &lineStart,
