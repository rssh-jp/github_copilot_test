...
bool CollisionDomainService::canMoveTo(
    const UnitEntity &unit, const Position &targetPosition,
  ...
  for (const auto &otherUnit : allUnits) {
    if (otherUnit.get() == &unit)
      continue;
    float combinedRadius = unit.getStats().getCollisionRadius() +
                           otherUnit->getStats().getCollisionRadius();
    if (isPointInCircle(targetPosition, otherUnit->getPosition(),
                        combinedRadius)) {
      return false;
    }
  }
...
Position CollisionDomainService::calculateAvoidancePosition(
  ...
  for (int i = 0; i < searchSteps; ++i) {
    ...
    Position candidatePosition(targetPosition.getX() + offsetX,
...
bool CollisionDomainService::hasCollisionAt(
    ...
    const std::vector<std::shared_ptr<UnitEntity>> &allUnits,
    const UnitEntity *excludeUnit, float movingRadius) {
...
bool CollisionDomainService::hasCollisionOnPath(
  ...
  for (const auto &otherUnit : allUnits) {
    ...
    float otherRadius = otherUnit->getStats().getCollisionRadius();
    float combined = movingRadius + otherRadius;
    if (isLineSegmentIntersectingCircle(start, end, otherUnit->getPosition(),
                                        combined)) {
      return true;
    }
  }
...
bool CollisionDomainService::isPointInCircle(const Position &point,
...
bool CollisionDomainService::isLineSegmentIntersectingCircle(
    ...
    const Position &circleCenter, float radius) {
  ...
  // 円の中心から線分への最短距離を計算
  float t = std::max(
      0.0f, std::min(1.0f, ((circleCenter.getX() - lineStart.getX()) * dx +
                            (circleCenter.getY() - lineStart.getY()) * dy) /
                               lengthSquared));

  // 線分上の最近点
  Position closestPoint(lineStart.getX() + t * dx, lineStart.getY() + t * dy);
...
bool CollisionDomainService::findFirstContactOnPath(
    const Position &start, const Position &end,
    const std::vector<std::shared_ptr<UnitEntity>> &allUnits,
    Position &outContactPosition, const UnitEntity *&outContactUnit,
    const UnitEntity *excludeUnit, float movingRadius) {
  ...
  for (const auto &otherPtr : allUnits) {
    ...
    // If start is inside (t1 < 0 < t2), treat contact as t=0 (already
    // overlapping)
    if (t1 < 0.0f && t2 >= 0.0f) {
      candidateT = 0.0f;
    } else {
      if (t1 >= -T_EPS && t1 <= 1.0f + T_EPS)
        candidateT = std::max(0.0f, std::min(1.0f, t1));
      else if (t2 >= -T_EPS && t2 <= 1.0f + T_EPS)
        candidateT = std::max(0.0f, std::min(1.0f, t2));
    }
    ...
    if (candidateT <= 1.0f + T_EPS) {
      if (candidateT < bestT) {
        ...
        bestPos = Position(start.getX() + dx * clampedT,
                           start.getY() + dy * clampedT);
        bestUnit = other;
        found = true;
      }
    }
