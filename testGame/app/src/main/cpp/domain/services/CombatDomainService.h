...
public:
  ...
  struct CombatResult {
    ...
    CombatResult()
        : damageDealt(0), targetKilled(false), attackerKilled(false) {}
    CombatResult(int damage, bool killed, bool counterKilled = false)
        : damageDealt(damage), targetKilled(killed),
          attackerKilled(counterKilled) {}
  };
   ...
   * @return 与えるダメージ
   */
  static int calculateDamage(const UnitStats &attackerStats,
   ...
   * @return 攻撃範囲内かどうか
   */
  static bool isInAttackRange(const UnitEntity &attacker,
