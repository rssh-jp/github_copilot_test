...
public:
   ...
   */
  UnitStats(int maxHp, int currentHp, int minAttackPower, int maxAttackPower,
            float moveSpeed, float attackRange, float attackSpeed,
            float collisionRadius = 0.1f)
      : maxHp_(maxHp), currentHp_(currentHp), minAttackPower_(minAttackPower),
        maxAttackPower_(maxAttackPower), moveSpeed_(moveSpeed),
        attackRange_(attackRange), attackSpeed_(attackSpeed),
    ...
    if (maxHp_ < 1)
      maxHp_ = 1;
    if (minAttackPower_ < 0)
      minAttackPower_ = 0;
    if (maxAttackPower_ < minAttackPower_)
      maxAttackPower_ = minAttackPower_;
    if (moveSpeed_ < 0.0f)
      moveSpeed_ = 0.0f;
    if (attackRange_ < 0.0f)
      attackRange_ = 0.0f;
    if (attackSpeed_ < 0.01f)
      attackSpeed_ = 0.01f;
  }
  ...
  int getRandomAttackPower() const {
    if (minAttackPower_ == maxAttackPower_)
  ...
  UnitStats takeDamage(int damage) const {
    ...
    UnitStats s(maxHp_, newHp, minAttackPower_, maxAttackPower_, moveSpeed_,
                attackRange_, attackSpeed_);
  ...
  UnitStats withAttackPower(int newMinAttack, int newMaxAttack) const {
    UnitStats s(maxHp_, currentHp_, newMinAttack, newMaxAttack, moveSpeed_,
                attackRange_, attackSpeed_);
    s.collisionRadius_ = collisionRadius_;
    return s;
  }
  ...
  UnitStats withMoveSpeed(float newMoveSpeed) const {
    UnitStats s(maxHp_, currentHp_, minAttackPower_, maxAttackPower_,
                newMoveSpeed, attackRange_, attackSpeed_);
    s.collisionRadius_ = collisionRadius_;
    return s;
  }
  ...
  bool operator==(const UnitStats &other) const {
    return maxHp_ == other.maxHp_ && currentHp_ == other.currentHp_ &&
  ...
  bool operator!=(const UnitStats &other) const { return !(*this == other); }
