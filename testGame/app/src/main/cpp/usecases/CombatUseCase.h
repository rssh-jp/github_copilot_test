 ...
 *
 * Contract:
 * - Caller provides a reference to the in-memory unit list that this use-case
 * will operate on.
 * - All returned UnitEntity pointers are non-owning references to objects
 * managed elsewhere.
 */
...
public:
   ...
   */
  using CombatEventCallback =
      std::function<void(const UnitEntity &attacker, const UnitEntity &target,
