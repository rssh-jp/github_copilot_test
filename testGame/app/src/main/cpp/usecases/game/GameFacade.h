...
public:
    ...
    GameFacade(std::shared_ptr<IUnitRepository> unitRepository,
        ...
        : unitRepository_(unitRepository),
    ...
    MoveResult stopPlayerUnit() {
        ...
        // 現在位置を目標位置に設定（停止）
        return moveUseCase_.setTargetPosition(
            playerUnit->getId(), 
     ...
     */
    std::vector<std::shared_ptr<UnitEntity>> getUnitsNear(const Position& position, float radius) {
