...
public:
   ...
   */
  std::pair<Position, MoveResult> calculateSafePosition(int unitId,
                                                        float deltaTime) {
    auto unit = unitRepository_->findById(unitId);
    if (!unit) {
  ...
  static std::string moveResultToString(MoveResult result) {
    ...
    case MoveResult::SUCCESS:
      return "SUCCESS";
    case MoveResult::UNIT_NOT_FOUND:
      return "UNIT_NOT_FOUND";
    case MoveResult::UNIT_CANNOT_MOVE:
      return "UNIT_CANNOT_MOVE";
    case MoveResult::COLLISION_DETECTED:
      return "COLLISION_DETECTED";
    case MoveResult::INVALID_POSITION:
      return "INVALID_POSITION";
    case MoveResult::MOVE_TOO_FAR:
      return "MOVE_TOO_FAR";
    default:
      return "UNKNOWN";
    }
...
private:
   ...
   * @brief 次のフレームでの位置を計算
   */
  Position calculateNextPosition(const UnitEntity &unit,
    ...
    return currentPos.moveBy(normalizedX * maxDistance,
   ...
   * @brief 安全な移動位置を探す
   */
  Position findSafePosition(const UnitEntity &unit,
                            const Position &desiredPosition) const {
    Position currentPos = unit.getPosition();
    ...
    for (int i = steps - 1; i >= 0; --i) {
      ...
      Position testPos =
          Position(currentPos.getX() +
                       (desiredPosition.getX() - currentPos.getX()) * ratio,
                   currentPos.getY() +
                       (desiredPosition.getY() - currentPos.getY()) * ratio);
