...
struct CameraState {
  ...
  CameraState(float x, float y, float zoom)
      : offsetX(x), offsetY(y), zoomLevel(zoom) {}
};
...
public:
   ...
   */
  using CameraStateChangeCallback =
      std::function<void(const CameraState &newState)>;
   ...
   * @param scale
   * ズームスケール（1.0が変化なし、1.0より大きいとズームイン、小さいとズームアウト）
