...
public:
   ...
   */
  using MovementEventCallback =
      std::function<void(const UnitEntity &unit, const Position &fromPosition,
                         const Position &toPosition)>;

  /**
   * @brief 移動失敗イベントのコールバック型
   */
  using MovementFailedCallback =
      std::function<void(const UnitEntity &unit, const Position &targetPosition,
