    ...
    if (unitRenderer_) {
        ...
        if (units_.size() >= 3) {
            ...
            if (unit2->isAlive() && unit3->isAlive()) {
                ...
                if (distance <= Unit::getCollisionRadius() * 2.0f) {
...
void Renderer::createModels() {
    ...
    float combatDistance = Unit::getCollisionRadius() * 2.0f + 0.01f;
    
    // ログ出力で確認
    aout << "Initial distance between units: " << distance << std::endl;
    aout << "Combat distance: " << combatDistance << std::endl;
    aout << "Collision radius: " << Unit::getCollisionRadius() << std::endl;
...
void Renderer::moveUnitToPosition(float x, float y) {
    ...
    std::shared_ptr<Unit> tappedUnit = nullptr;
