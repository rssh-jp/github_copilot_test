...
public:
   ...
   */
  static std::shared_ptr<UnitEntity>
  fromLegacyUnit(const std::shared_ptr<Unit> &legacyUnit) {
    if (!legacyUnit) {
      return nullptr;
    }
    ...
    // ステータスの変換
    UnitStats stats(legacyUnit->getMaxHP(), legacyUnit->getCurrentHP(),
    ...
    auto unitEntity = std::make_shared<UnitEntity>(
        legacyUnit->getId(), legacyUnit->getName(), position, stats);
   ...
   */
  static void
  updateLegacyUnits(const std::vector<std::shared_ptr<UnitEntity>> &entities,
