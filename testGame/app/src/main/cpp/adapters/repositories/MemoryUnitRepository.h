...
public:
   ...
   */
  std::vector<std::shared_ptr<UnitEntity>> findInRange(const Position &center,
    ...
    for (const auto &pair : units_) {
      ...
      float distanceSquared =
          calculateDistanceSquared(center, pair.second->getPosition());
...
// デバッグログ
#ifdef DEBUG
    aout << "Saved unit: " << unit->getName() << " (ID: " << id << ") at ("
   ...
   * @brief 全ユニットを削除
   */
  void removeAll() override { units_.clear(); }

  /**
   * @brief ユニット数を取得
   */
  size_t count() const override { return units_.size(); }
   ...
   */
  std::shared_ptr<UnitEntity> findNearest(const Position &position,
                                          int excludeId = -1) {
    ...
    for (const auto &pair : units_) {
      ...
      float distanceSquared =
          calculateDistanceSquared(position, pair.second->getPosition());
...
private:
   ...
   */
  float calculateDistanceSquared(const Position &pos1,
