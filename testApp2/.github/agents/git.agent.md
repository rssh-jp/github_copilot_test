...
すべての `git` および `gh` コマンドは **WSL（Windows Subsystem for Linux）経由** で実行してください。
ターミナルで実行する場合は必ず WSL ターミナル（`wsl` コマンドまたは WSL セッション）を使用し、PowerShell や cmd から直接 git を呼ばないこと。
