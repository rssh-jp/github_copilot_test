...
9. **ユーザー確認フェーズ**: 実装内容とリリースビルド結果をユーザーに報告し、PR 作成の可否を確認する
   - 「PR を作成してください」の指示があれば次のステップへ進む
   - 追加の修正依頼があった場合: ステップ 3（impl）へ戻り、依頼内容を prompt に含めて対応し、その後ステップ 4〜9 を再実施する
   - 「PR は不要」「後で行う」などの指示があればここで完了とする
10. #tool:agent で `git` エージェントを呼び出し、ブランチをプッシュして PR を作成する（push-pr モード）
11. PR の URL をユーザーに通知する
...
| 確認フェーズで追加修正 | impl（ステップ 3） | 3 → 4 → 5 → 6 → 7 → 8 → 9 |
